...
public final class QueryUtils {
    ...
    /**
     * Return a list of {@link CustomModel} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<CustomModel> extractEarthquakes() {

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<CustomModel> earthquakes = new ArrayList<>();
        ...
        try {
            ...
            for(int i=0;i<feature.length();i++) {
                JSONObject feature1 = feature.getJSONObject(i);
                ...
                Date dateo = new Date(timeinmilliseconds);
                SimpleDateFormat dateFormat = new SimpleDateFormat("MMM DD,YYYY");
                String date = dateFormat.format(dateo);
                SimpleDateFormat timeFormat = new SimpleDateFormat("h:mm a");
                String time = timeFormat.format(dateo);
                String url = properties.getString("url");
                earthquakes.add(new CustomModel(mag,place,date,time,url));
